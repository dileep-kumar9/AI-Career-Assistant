...
def test_agent_prepare_end_to_end():
    r = client.post("/ai/agent/prepare", json={
        "jd": "We need a python backend engineer with sql and docker experience.",
        "resume": "I am a python developer with sql and docker experience.",
    })
    assert r.status_code == 200
    body = r.json()
...
def test_interview_answer_persists():
    ...
    r = client.post("/interview/answer", json={
        "user_id": uid, "interview_type": "HR", "mode": "typing",
        "question": "Tell me about yourself.", "answer": "I am a backend engineer with 3 years of experience.",
        "expected_topics": [],
    })
    assert r.status_code == 200
...
def test_guest_resume_tailor_no_account_needed():
    r = client.post("/resume-tools/tailor", json={
        "master_resume_text": "Python developer with django and sql experience.",
        "job_description": "Need a python developer with django, sql, and docker.",
    })
    assert r.status_code == 200
...
def test_guest_parse_preview():
    r = client.post("/resume-tools/parse-preview", json={
...
def test_promote_tailored_resume_to_master():
    ...
    r = client.get(f"/users/{uid}/resumes/")
    assert len(r.json()) == 1  # still just one master row, not a duplicate


def test_google_signin_returns_503_when_not_configured():
    # GOOGLE_CLIENT_ID is blank in test env -- endpoint should fail clearly,
    # not crash, when Google sign-in isn't configured.
...
def test_autofill_resolves_saved_resume_without_crashing():
    ...
    assert r.status_code == 200
    # No Playwright browser is installed in CI/this sandbox -- the important
    # thing is it resolves cleanly (doesn't 500) rather than what the exact
    # browser-launch error says.
